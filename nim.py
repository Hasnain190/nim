...
class NimAI():
    ...
    def __init__(self, alpha=0.5, epsilon=0.1):
        ...
        """
        self.q = dict() 
        # For example, if we wanted to set the Q-value of 
        # the state [0, 0, 0, 2] and the action (3, 2) to -1, 
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        updated_q = old_q + self.alpha*(old_q - new_estimate)
    ...
    def best_future_reward(self, state:list):
        ...
        `state`, return 0.
        """
        # first we will get all the available state,action pairs in a given state
        ...
        for pair in list(state_action_pairs):
            q_values.append( self.q.get(pair,0) )
    ...
    def choose_action(self, state, epsilon=True)->tuple:
        """                                 
        ...
        if  epsilon:
             if random.random() < self.epsilon:
                ...
                q_values = []
                for pair in list(actions):
                    q_values.append(self.q.get(pair,0))
                    
                
                max_q = max(q_values) 
            
                best_action = list(actions).pop(max_q)
